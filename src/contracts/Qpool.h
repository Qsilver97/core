...
#define QPOOL_QPOOL_CONTRACTID _mm256_set_epi32(6, 0, 0, 0, 0, 0, 0, 0)
...
public:
	...
	_
		...
		PUBLIC_PROCEDURE(GetEnableToken) if (input.NumberOfToken >= state.NumberOfEnableToken) return;
	tokenInfor token = state.TokenList.get(input.NumberOfToken);
	output.assetName = token.assetName;
	output.issuer = token.issuer;
	_

	PUBLIC_PROCEDURE(CreateLiquidityPool) if (qpi.invocationReward() < QPOOL_FEE_CREATE_POOL + QPOOL_TOKEN_TRANSER_FEE * (input.NumberOfToken - 1) + input.initialAmountOfQU)
	{
		if (qpi.invocationReward() > 0)
		{
			qpi.transfer(qpi.invocator(), qpi.invocationReward());
	...
	if (input.NumberOfToken > 2)
		qpi.transferShareOwnershipAndPossession(assetnameOftoken1, issuerOfToken1, qpi.invocator(), qpi.invocator(), input.initialAmount1, QPOOL_QPOOL_CONTRACTID);
	if (input.NumberOfToken > 3)
		qpi.transferShareOwnershipAndPossession(assetnameOftoken2, issuerOfToken2, qpi.invocator(), qpi.invocator(), input.initialAmount2, QPOOL_QPOOL_CONTRACTID);
	if (input.NumberOfToken > 4)
		qpi.transferShareOwnershipAndPossession(assetnameOftoken3, issuerOfToken3, qpi.invocator(), qpi.invocator(), input.initialAmount3, QPOOL_QPOOL_CONTRACTID);
	qpi.transferShareOwnershipAndPossession(QPOOL_QWALLET_TOKEN, IssuerQWALLET, qpi.invocator(), qpi.invocator(), input.initialAmountOfQWALLET, QPOOL_QPOOL_CONTRACTID);
	...
	PUBLIC_PROCEDURE(IssueAsset) if (qpi.invocationReward() < QPOOL_FEE_ISSUE_ASSET)
	{
		if (qpi.invocationReward() > 0)
		{
			qpi.transfer(qpi.invocator(), qpi.invocationReward());
	...
	_
		...
		PUBLIC_PROCEDURE(PoolList) if (input.NumberOfPool >= state.NumberOfPool) return;
	PoolInfo pool;
	...
	REGISTER_USER_PROCEDURE(PoolList, 4);
	REGISTER_USER_PROCEDURE(GetEnableToken, 5);
	_

		INITIALIZE
			state.NumberOfPool = 0;
	state.NumberOfEnableToken = 1;
	tokenInfor QwalletToken;
	QwalletToken.assetName = QPOOL_QWALLET_TOKEN;
	QwalletToken.issuer = QPOOL_ISSUER_QWALLET;
	state.TokenList.set(0, QwalletToken);
	_
