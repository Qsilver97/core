...
#define	QPOOL_QWALLET_CONTRACTID _mm256_set_epi32(6, 0, 0, 0, 0, 0, 0, 0)
...
public:
	...
	PUBLIC_PROCEDURE(GetEnableToken)
	...
	PUBLIC_PROCEDURE(CreateLiquidityPool)
		...
		if(input.NumberOfToken > 2) qpi.transferShareOwnershipAndPossession(assetnameOftoken1, issuerOfToken1, qpi.invocator(), qpi.invocator(), input.initialAmount1, QPOOL_QWALLET_CONTRACTID);
		if(input.NumberOfToken > 3) qpi.transferShareOwnershipAndPossession(assetnameOftoken2, issuerOfToken2, qpi.invocator(), qpi.invocator(), input.initialAmount2, QPOOL_QWALLET_CONTRACTID);
		if(input.NumberOfToken > 4) qpi.transferShareOwnershipAndPossession(assetnameOftoken3, issuerOfToken3, qpi.invocator(), qpi.invocator(), input.initialAmount3, QPOOL_QWALLET_CONTRACTID);
		qpi.transferShareOwnershipAndPossession(QPOOL_QWALLET_TOKEN, IssuerQWALLET, qpi.invocator(), qpi.invocator(), input.initialAmountOfQWALLET, QPOOL_QWALLET_CONTRACTID);
...
//		add the number of pool
	...
	INITIALIZE
		...
		QwalletToken.issuer = QPOOL_QWALLET_CONTRACTID;
